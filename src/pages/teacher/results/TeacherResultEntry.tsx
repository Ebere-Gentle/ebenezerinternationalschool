...
export default function TeacherResultEntry({ assessmentType }: Props) {
  ...
  const availableSubjects = useMemo(() => {
    ...
    return assignments.filter((item) => item.class_id === classId && !seen.has(item.subject_id) && seen.add(item.subject_id));
  }, [assignments, classId]);
  const availableClasses = useMemo(() => {
    const seen = new Set<string>();
    return assignments.filter((item) => !seen.has(item.class_id) && seen.add(item.class_id));
  }, [assignments]);
  ...
  async function loadContextAndAssignments() {
    ...
    try {
      ...
      if (!sessionData?.id || !termData?.id || !teacher?.id) {
  ...
  async function save() {
    ...
    try {
      ...
      if (!batchId) {
        ...
        const { error: batchUpdateError } = await supabase.from('result_batches').update({ max_score: maximum, weight: maximum, updated_at: new Date().toISOString(), updated_by: user?.id }).eq('id', batchId);
      ...
      const { error: submitError } = await supabase.from('result_batches').update({ status: 'submitted' }).eq('id', batchId);
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4">
    ...
    <section className="grid gap-4 rounded-2xl border bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800 md:grid-cols-3">
      <label className="text-sm font-semibold">Class<select value={classId} onChange={(event) => { setClassId(event.target.value); setSubjectId(''); }} className="mt-2 w-full rounded-xl border px-3 py-3 dark:border-slate-600 dark:bg-slate-900"><option value="">Select assigned class</option>{availableClasses.map((item) => <option key={item.class_id} value={item.class_id}>{item.class_name}</option>)}</select></label>
      ...
      <div className="rounded-xl border bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-900"><p className="text-xs font-semibold uppercase text-slate-500">Maximum mark</p><p className="mt-1 text-2xl font-bold">{maximum || 'Not configured'}</p><p className="text-xs text-slate-500">Admin configuration · total = 100</p></div>
    </section>

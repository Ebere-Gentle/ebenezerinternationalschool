...
export default function TeacherResultEntry({ assessmentType }: Props) {
  ...
  useEffect(() => { if (sessionId && termId && classId) void loadConfig(); else { setConfig(null); setAssessmentGroup(null); } }, [sessionId, termId, classId]);
  ...
  async function loadConfig() {
    ...
    const { data, error } = await supabase.from('result_assessment_configs').select('first_test_max,second_test_max,exam_max,total_max,components').eq('academic_session_id', sessionId).eq('term_id', termId).eq('academic_group', group).eq('status', 'active').maybeSingle();
    if (error) { toast.error(error.message); setConfig(null); return; }
    if (!data) { setConfig(null); return; }
    const components = Array.isArray(data.components) ? data.components : [];
    const findMax = (key: string, fallback: number) => Number(components.find((item: any) => item?.key === key)?.max_score ?? fallback);
    setConfig({ first_test_max: findMax('first_test', Number(data.first_test_max)), second_test_max: findMax('second_test', Number(data.second_test_max)), exam_max: findMax('exam', Number(data.exam_max)), total_max: Number(data.total_max || 100) });
  }

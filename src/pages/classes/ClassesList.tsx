...
// Database class_level_type does NOT contain graduate.
// Graduate class is stored as level = senior and identified by name/code.
const VALID_LEVELS = [
  'creche',
  'nursery',
  'primary',
...
const ClassesList: React.FC = () => {
  ...
  const [showClassDetails, setShowClassDetails] =
    useState(false);
  ...
  const [showAddSubjectModal, setShowAddSubjectModal] =
    useState(false);
  ...
  const [showOrphanCleanup, setShowOrphanCleanup] =
    useState(false);

  const [orphanRecords, setOrphanRecords] =
    useState<any[]>([]);
  ...
  const isGraduateClass = (cls: ClassItem | null) => {
    ...
    return (
      cls.name?.toLowerCase().includes('graduate') ||
  ...
  // =========================================================
  // CHECK ORPHANED TEACHER ASSIGNMENTS
  // =========================================================
  ...
  const checkOrphanedRecords = async (
    ...
    try {
      ...
      const orphans =
        orphanChecks.filter(Boolean);

      setOrphanRecords(orphans);
      setShowOrphanCleanup(
        orphans.length > 0
      );
  ...
  const cleanupOrphanedRecords = async () => {
    ...
    if (
      !window.confirm(
        `Delete ${orphanRecords.length} orphaned teacher assignment(s)?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.success(
        `Cleaned up ${ids.length} orphaned assignment(s)`
      );
      ...
      if (selectedClass) {
        await viewClassDetails(
          selectedClass
        );
      }
    } catch (error: any) {
      console.error(
        'Error cleaning up orphans:',
        error
      );

      toast.error(
        error?.message ||
          'Failed to clean up orphaned records'
      );
    }
  };
  ...
  const fetchClasses = async () => {
    ...
    try {
      ...
      // IMPORTANT:
      // class_subjects is the source of truth.
      // --------------------------------------------

      const {
        data: curriculumAssignments
      } = await supabase
        .from('class_subjects')
        .select(
          'class_id, subject_id'
  ...
  const fetchTeachers = async () => {
    ...
    try {
      const {
        ...
        .from('teachers')
        .select(
          'id, teacher_id, first_name, last_name, email, phone_number'
        )
        .eq(
          'branch_id',
          userBranchId
    ...
    async () => {
      ...
      try {
        ...
        let query = supabase
          .from('students')
          .select(
            'id, first_name, last_name, admission_number, secondary_admission_number, student_id, current_status'
          )
          .eq(
            'branch_id',
            userBranchId
  ...
  const handleDeleteClass =
    async (id: string) => {
      ...
      if (
        !window.confirm(
          `Delete ${cls?.name || 'this class'}?\n\nStudents will be unassigned from the class. Curriculum and teacher assignments will also be removed.`
        )
      ) {
        return;
      }
      ...
      try {
        ...
        let studentQuery =
          supabase
            .from('students')
            .select(
              'id, first_name, last_name, admission_number, secondary_admission_number, student_id, email, phone_number, gender, date_of_birth, current_status'
            )
            .eq(
              'class_id',
              cls.id
        ...
        // --------------------------------------------
        // CURRICULUM SUBJECTS
        // Source of truth = class_subjects
        // --------------------------------------------
        ...
        const {
          ...
          .from('subjects')
          .select(
            'id, subject_id, name, code, description, branch_id'
          )
          .in(
            'id',
            subjectIds
  ...
  const handleAddStudent =
    async () => {
      ...
      try {
        ...
        // Close previous current class history
        await supabase
    ...
    async () => {
      ...
      try {
        ...
        if (
          ...
          throw classSubjectCheckError;
        }

        // --------------------------------------------
        // Create curriculum assignment if missing
        // --------------------------------------------
        ...
        if (
          ...
          if (
            ...
            throw classSubjectInsertError;
          }
        }

        // --------------------------------------------
        // If teacher selected, add teacher assignment
        // --------------------------------------------
  ...
  const handleRemoveSubject =
    async (
      ...
      if (
        !window.confirm(
          `Remove ${subjectName} from ${selectedClass.name}?\n\nThis will also remove any teacher assignment for this subject in this class.`
        )
      ) {
        return;
      }
      ...
      try {
        ...
        toast.success(
          `${subjectName} removed from ${selectedClass.name}`
        );
          ...
          (
            ...
            return (
              <motion.div
                ...
                className={`bg-white/70 dark:bg-gray-800/70 rounded-2xl border shadow-xl hover:shadow-2xl transition-all ${
                  graduate
                    ? 'border-green-500/30'
  ...
  const UserCheckIcon =
    () => (
      <svg
        viewBox="0 0 24 24"
        className="w-4 h-4 mx-auto text-orange-500"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
      >
    ...
    <div className="space-y-6">
        ...
        orphanRecords.length >
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-800 dark:text-amber-300">
                  ...
                  orphaned teacher assignment(s)
                </p>

                <p className="text-xs text-amber-600 mt-1">
                  These records reference missing classes, subjects, or teachers.
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-sm flex items-center gap-1"
            >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Manage classes, curriculum subjects, teachers and students
          </p>
        </div>
        ...
        <button
          ...
          className="px-4 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-medium shadow-lg flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          className="from-purple-500 to-violet-500"
        />

        <StatCard
          label="Class Teacher Coverage"
          value={`${Math.round(
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <button
          ...
          className="p-2.5 border border-gray-200 dark:border-gray-700 rounded-xl"
          title="Refresh"
        >
            ...
            >
              ...
              <div className="p-6 space-y-4">
                <input
                  ...
                  className="w-full px-4 py-2.5 border rounded-xl dark:bg-gray-900 dark:text-white"
                />
                ...
                <div className="flex gap-3 pt-3">
                  ...
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {saving ? (
        ...
        {showClassDetails &&
          selectedClass && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
              ...
              >
                <div className="sticky top-0 z-10 bg-white dark:bg-gray-800 border-b p-4 flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                      {
                        selectedClass.name
                  ...
                  {loadingDetails ? (
                    ...
                    <>
                      ...
                      <div>
                        ...
                        {classStudents.length ===
                        0 ? (
                          <div className="p-6 text-center bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                            <p className="text-sm text-gray-500">
                      ...
                      <div className="border-t pt-5">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <button
                            ...
                            className="text-xs px-3 py-1.5 bg-purple-600 text-white rounded-lg flex items-center gap-1"
                          >
                        ...
                        0 ? (
                          <div className="text-center py-8 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                            ...
                            <p className="text-sm text-gray-500">
                              No curriculum subjects assigned.
                            </p>
                          </div>
                        ) : (
                            ...
                            {classSubjects.map(
                              item => (
                                <div
                                  ...
                                  className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl border border-gray-200 dark:border-gray-700"
                                >
              ...
              >
                ...
                <div className="flex gap-3 mt-5">
                  ...
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 bg-green-600 text-white rounded-xl disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {addingStudent ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ) : (
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                    <p className="text-xs text-blue-700 dark:text-blue-300">
                      The subject is assigned to the class curriculum first. A teacher can be assigned now or later.
                    </p>
                  </div>
                  ...
                  <div className="flex gap-3 pt-3">
                    ...
                    <button
                      ...
                      className="flex-1 px-4 py-2.5 bg-purple-600 text-white rounded-xl disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {saving ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
  ...
  return (
    <div className="bg-white/70 dark:bg-gray-800/70 rounded-2xl p-4 border border-gray-200 dark:border-gray-700 shadow-xl">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`w-10 h-10 rounded-xl bg-gradient-to-br ${className} flex items-center justify-center shadow-lg`}
        >
          {icon}
        </div>
      </div>

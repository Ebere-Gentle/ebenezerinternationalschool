...
const Login: React.FC = () => {
  ...
  const getInitialStory = (): number => {
    try {
      ...
      if (savedIndex === null || savedTimestamp === null) {
        ...
        localStorage.setItem(STORY_TIME_KEY, Date.now().toString());
        return 0;
      }
      ...
      if (elapsed >= STORY_DURATION) {
        ...
        localStorage.setItem(STORY_INDEX_KEY, nextIndex.toString());
        localStorage.setItem(STORY_TIME_KEY, Date.now().toString());
        return nextIndex;
      }
      ...
      console.error('Unable to load login story:', error);
      return 0;
    }
  };
  ...
  const [currentStory, setCurrentStory] = useState<number>(getInitialStory);
  ...
  useEffect(() => {
    const checkStoryExpiration = () => {
      try {
        ...
        const savedIndex = Number(localStorage.getItem(STORY_INDEX_KEY));

        if (Number.isNaN(timestamp) || Number.isNaN(savedIndex)) {
          return;
        }

        const elapsed = Date.now() - timestamp;
        if (elapsed >= STORY_DURATION) {
          const nextIndex = (savedIndex + 1) % storyImages.length;
          localStorage.setItem(STORY_INDEX_KEY, nextIndex.toString());
          localStorage.setItem(STORY_TIME_KEY, Date.now().toString());
          setCurrentStory(nextIndex);
        }
      } catch (error) {
        console.error('Story expiration check failed:', error);
      }
    };

    checkStoryExpiration();
    const interval = window.setInterval(checkStoryExpiration, 60 * 1000);
  ...
  const nextStory = (currentStory + 1) % storyImages.length;
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full h-12 rounded-lg border ${
                    resetError ? 'border-red-300' : 'border-gray-200'
  ...
  // ==========================================================
  // RENDER RESET PASSWORD (Set New Password)
  // ==========================================================
          ...
          <>
            ...
            <form onSubmit={handleResetPassword} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={resetting}
                className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-green-600 font-semibold text-white shadow-lg shadow-green-600/30 transition hover:bg-green-700 hover:shadow-green-700/40 disabled:cursor-not-allowed disabled:bg-green-400"
              >
                {resetting ? (
                  <>
                    <Loader2 className="h-5 w-5 animate-spin" />

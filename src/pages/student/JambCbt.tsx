...
import type {
  ...
  JambRegistration,
  JambSubject,
} from '../../services/jamb/jambCbt.service';

const Sparkline: React.FC<{
  values: number[];
  color?: string;
  height?: number;
}> = ({ values, color = '#4f46e5', height = 48 }) => {
  if (!values.length) {
    return (
  ...
  const area =
    ...
    points
      ...
      .join(' ') +
    ` L ${
      (pad + (values.length - 1) * stepX).toFixed(1)
    } ${h - pad} Z`;
...
/**
 * JAMB CBT eligibility
 *
 * This feature is intentionally restricted to:
 *
 *   - SS2 Science
 *   - SS2 Arts
 *   - SS3 Science
 ...
 *
 * Only ACTIVE students in SS2/SS3 Science or Arts
 * are allowed to use JAMB CBT.
 */
const isEligibleForJambCbt = (student: any): boolean => {
  ...
  if (!['science', 'arts'].includes(department)) {
    return false;
  }
  ...
  /**
   * Accept common representations used by the school system:
   * SS2
   * SS 2
   * Senior 2
   * Senior Two
   *
   * We deliberately do NOT activate the feature merely because
   * a student has `senior` level. The student must specifically
   * be SS2 or SS3.
   */
  const isSS2 =
    /\bss\s*2\b/.test(className) ||
    /\bsenior\s*2\b/.test(className) ||
    /\bsenior\s*two\b/.test(className);

  const isSS3 =
    /\bss\s*3\b/.test(className) ||
    /\bsenior\s*3\b/.test(className) ||
    /\bsenior\s*three\b/.test(className);
...
const JambCbt: React.FC = () => {
  ...
  const load = async () => {
    try {
      ...
      /*
       * Load the student first.
       *
       * We intentionally do not load JAMB subjects,
       * registrations or attempts for students who are
       * not eligible for the JAMB CBT programme.
       */
      const s = await jambCbtService.getStudent();

      setStudent(s);

      if (!isEligibleForJambCbt(s)) {
        setSubjects([]);
      ...
      const allSubjects =
        await jambCbtService.getSubjects();

      setSubjects(allSubjects);

      const reg =
        await jambCbtService.getRegistration(s.id);

      setRegistration(reg);

      if (reg) {
        setAttempts(
          await jambCbtService.getAttempts(reg.id)
        );
      } else {
        setAttempts([]);
      }
  ...
  useEffect(() => {
    if (!registration || !eligible) return;
    ...
    const loadCooldowns = async () => {
      const entries = await Promise.all(
        (
          registration.jamb_registration_subjects ||
          []
        ).map(async (s) => {
          try {
            const info =
              await jambCbtService.getCooldownInfo(
                registration.id,
                s.subject_id
              );

            const remainingMs =
              info.nextAvailableAt
                ? Math.max(
                    0,
                    info.nextAvailableAt.getTime() -
                      Date.now()
                  )
                : 0;
  ...
  const register = async () => {
    if (!eligible) {
      toast.error(
        'JAMB CBT is available only to active SS2 and SS3 Science or Arts students.'
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        'JAMB CBT registration completed and locked.'
      );
    } catch (error: any) {
      toast.error(
        error?.message ||
  ...
  const subjectStats = useMemo(() => {
    ...
    return registration.jamb_registration_subjects.map(
      (item) => {
        const subjAttempts = attempts.filter(
          (a) =>
            a.subject?.id === item.subject_id
        );
  ...
  const trendDelta = useMemo(() => {
    ...
    const older =
      chronological.slice(0, mid);

    const newer =
      chronological.slice(mid);
  ...
  const PractiseButton: React.FC<{
      ...
      return (
        <div className="mt-4 w-full">
          ...
          <p className="text-[10px] text-gray-500 text-center mt-1.5">
            Next:{' '}
            {cd.nextAvailableAt?.toLocaleString()}
          </p>
        </div>
      );
    }
    ...
    return (
      <button
        onClick={() =>
          navigate(
            `/student/jamb-cbt/test/${subjectId}`
          )
        }
   ...
   * Hard UI guard.
   *
   * This is deliberately after loading the student but
   * before rendering any JAMB registration or analytics.
   *
   * Ineligible students therefore cannot register,
   * practise, or see the JAMB CBT dashboard.
   */
  if (!eligible) {
    return (
      <div className="space-y-6 pb-10">
          ...
          <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-5">
            <div>
              ...
              <p className="text-indigo-100 mt-2 max-w-2xl text-sm md:text-base">
                JAMB CBT preparation is currently
                available to eligible senior students
                in Science and Arts.
              </p>
            </div>
        ...
        <section className="max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 md:p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mt-5">
            JAMB CBT access is restricted
          </h2>

          <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 leading-relaxed">
            This preparation programme is currently
            available only to active SS2 and SS3
            students in the Science or Arts department.
          </p>
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-950 via-indigo-800 to-violet-700 p-6 md:p-8 text-white shadow-xl">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          <div>
            ...
            <p className="text-indigo-100 mt-2 max-w-2xl text-sm md:text-base">
              Register once, practise past-question
              sets, take timed CBTs and track your
            ...
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  ...
                  compulsory. Select two additional
                  subjects.
                </p>
              </div>
          ...
          <aside className="space-y-4">
            <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 p-5">
              ...
              <p className="text-[11px] text-gray-500 text-center mt-3 leading-relaxed">
                Once confirmed, your subject combination
                is locked for this academic session.
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-indigo-100 dark:border-indigo-900/40 bg-gradient-to-br from-indigo-50 to-violet-50 dark:from-indigo-900/20 dark:to-violet-900/20 p-5">
              ...
              <ul className="mt-3 space-y-2 text-xs text-indigo-900/80 dark:text-indigo-100/80">
                <li className="flex gap-2">
                  ...
                  Pick subjects aligned with your intended
                  course of study.
                </li>

                <li className="flex gap-2">
                  <ChevronRight className="w-3 h-3 mt-0.5 flex-shrink-0" />
                  English and Mathematics are required for
                  most programmes.
                </li>

                <li className="flex gap-2">
                  <ChevronRight className="w-3 h-3 mt-0.5 flex-shrink-0" />
                  Choose subjects you can dedicate consistent
                  practice time to.
                </li>
              </ul>
            </div>
            ...
            <section className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 p-5 md:p-7">
              <div className="flex items-start justify-between gap-3 mb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Score progression across your last{' '}
                    {scoreTrend.length} attempt
                    {scoreTrend.length === 1
                      ? ''
                      : 's'}
                  </p>
              ...
              {scoreTrend.length === 0 ? (
                <div className="py-10 text-center text-gray-400 text-sm">
                  Take your first CBT to unlock the
                  performance graph.
                </div>
              ) : (
                <>
            ...
            <section className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 p-5 md:p-7">
              ...
              {subjectBars.every(
                ...
                <p className="text-sm text-gray-500">
                  No attempts recorded yet. Start
                  practising to see per-subject
                  analytics.
                </p>
              ) : (
            ...
            <section className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 p-5 md:p-7">
              <div className="flex items-start justify-between gap-3 mb-5">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Topics where your accuracy is lowest.
                    These are your highest-impact study
                    areas.
                  </p>
                </div>
              ...
              {weakTopics.length === 0 ? (
                <div className="rounded-2xl border border-dashed dark:border-gray-700 p-6 text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-1 max-w-md mx-auto">
                    Topic-level insights appear once your
                    CBT attempts include per-topic answer
                    breakdowns. Keep practising to build
                    this profile.
                  </p>
                </div>
              ) : (
            ...
            <section className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 p-5 md:p-7">
              ...
              <div className="space-y-3">
                {recommendations.length ===
                  0 && (
                  <p className="text-sm text-gray-500">
                    Complete a few CBT attempts to receive
            ...
            <section className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 p-5 md:p-7">
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Registration is locked for this
                    academic session.
                  </p>
                </div>
              ...
              weakestSubject.attempts > 0 && (
                <div className="rounded-3xl border border-amber-200 dark:border-amber-900/40 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 p-5">
                  ...
                  <p className="text-xs text-amber-800/80 dark:text-amber-300/80 mt-1">
                    Lowest average at{' '}
                    {weakestSubject.avg.toFixed(
                      0
                    )}
                    % across{' '}
                    {
                      weakestSubject.attempts
                    }{' '}
                    attempt
                    {weakestSubject.attempts ===
                    1
                  ...
                  (item) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          navigate(
                            `/student/jamb-cbt/test/${item.subject_id}`
                          )
                        }

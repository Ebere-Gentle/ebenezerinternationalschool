...
const StudentViewExam: React.FC = () => {
  const { user } = useAuth();
  ...
  return (
    <div className="container mx-auto p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        {results.length > 0 ? <ResultTable results={results} title="Exam Results" /> : <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 text-center"><p className="text-gray-500 dark:text-gray-400">No exam results available yet.</p></div>}
      </motion.div>
    </div>
  );
};

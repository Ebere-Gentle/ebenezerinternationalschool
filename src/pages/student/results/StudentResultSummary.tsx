...
export default function StudentResultSummary() {
  const { user } = useAuth();
  const [student, setStudent] = useState<any>(null), [school, setSchool] = useState<any>(null), [className, setClassName] = useState('');
  const [rows, setRows] = useState<Row[]>([]), [context, setContext] = useState<Context | null>(null), [summary, setSummary] = useState<any>(null), [loading, setLoading] = useState(true);
  ...
  async function load() {
    ...
    try {
      ...
      const [{ data: branch }, { data: cls }, { data: session }] = await Promise.all([
        ...
        supabase.from('classes').select('name,level,department').eq('id', studentData.class_id).maybeSingle(),
        supabase.from('academic_sessions').select('id,session_name,term_name,term_number,is_current').eq('is_current', true).maybeSingle(),
      ]);
      setSchool(branch); setClassName(cls?.name || '');
      ...
      const group = resolveAssessmentGroup({ id: studentData.class_id, name: cls?.name || '', level: cls?.level, department: cls?.department });
      const { data: config, error: configError } = await supabase.from('result_assessment_configs').select('first_test_max,second_test_max,exam_max,total_max,components').eq('academic_session_id', session.id).eq('term_id', currentTerm.id).eq('academic_group', group).eq('status', 'active').maybeSingle();
      if (configError) throw configError;
      if (!config) throw new Error(`Assessment scheme for ${group.replace('_', ' ')} has not been configured for the current academic session and term.`);
      const components = Array.isArray(config.components) ? config.components : [];
      const componentMax = (key: string, fallback: number) => Number(components.find((item: any) => item?.key === key)?.max_score ?? fallback);
      const test1Max = componentMax('first_test', Number(config.first_test_max));
      const test2Max = componentMax('second_test', Number(config.second_test_max));
      const examMax = componentMax('exam', Number(config.exam_max));
      const totalMax = Number(config.total_max || test1Max + test2Max + examMax);
      setContext({ session: session.session_name, term: currentTerm.term, test1Max, test2Max, examMax, totalMax });
      ...
      for (const batch of batches || []) {
        ...
        existing.grade = entry?.grade || grade(percentage);
        existing.remark = entry?.remark || remark(percentage);
      ...
      const average = nextRows.length ? overallTotal / nextRows.length / totalMax * 100 : 0;
    ...
    } catch (error: any) { toast.error(error.message || 'Unable to load result sheet'); setRows([]); }
    finally { setLoading(false); }
  }

  const overall = useMemo(() => {
    if (!rows.length || !context?.totalMax) return 0;
    return rows.reduce((sum, row) => sum + (row.test1 || 0) + (row.test2 || 0) + (row.exam || 0), 0) / rows.length / context.totalMax * 100;
  }, [rows, context?.totalMax]);
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5 p-4 sm:p-6">
    <div className="grid gap-4 lg:grid-cols-[1fr_auto]"><div><p className="text-sm font-bold uppercase tracking-[0.2em] text-emerald-600">Academic records</p><h1 className="text-3xl font-black text-slate-900 dark:text-white">Official Result Sheet</h1><p className="mt-1 text-slate-500">Current session, term and assessment scheme are read from the database.</p></div><div className="flex gap-2"><button onClick={() => window.print()} className="flex items-center gap-2 rounded-xl border bg-white px-4 py-3 font-semibold shadow-sm"><Printer className="h-4 w-4" /> Print</button><button onClick={() => void load()} className="flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 font-semibold text-white"><RefreshCw className="h-4 w-4" /> Refresh</button></div></div>

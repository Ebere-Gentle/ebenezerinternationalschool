...
const configKey = (type: AssessmentType): keyof Config => ({
  first_test: 'first_test_max',
  second_test: 'second_test_max',
  exam: 'exam_max',
}[type]);
...
export default function AdminResultEntry({ assessmentType }: Props) {
  ...
  const loadContext = useCallback(async () => {
    ...
    try {
      ...
      if (!currentSession) {
        throw new Error('No current academic session is configured. Set one session as current first.');
      }
      ...
      const currentTerm = (activeTerms || []).find(
        (item) =>
          normalise(item.session) === normalise(currentSession.session_name) &&
          normalise(item.term) === normalise(currentSession.term_name),
      );

      if (!currentTerm) {
        throw new Error(
          `No active term matches ${currentSession.session_name} / ${currentSession.term_name || 'current term'}.`,
        );
      }
      ...
      if (!savedConfig) {
        throw new Error('Assessment components are not configured for the current session and term.');
      }
      ...
      setClasses(
        (classData || []).filter(
          (item) =>
            ...
            normalise(item.academic_session) === normalise(currentSession.session_name),
        ),
      );
    } catch (error: any) {
      setSession(null);
  ...
  const loadRegister = useCallback(async () => {
    ...
    try {
      ...
      setSubjects(
        (assignmentResult.data || []).flatMap((item: any) =>
  ...
  const loadExistingScores = useCallback(async () => {
    ...
    (entries || []).forEach((entry: any) => {
      next[entry.student_id] = entry.score == null ? '' : String(entry.score);
    });
    setScores(next);
  ...
  const enteredCount = students.filter(
    (student) => scores[student.id] !== '' && scores[student.id] != null,
  ).length;

  const averageEntered = enteredCount
    ? students.reduce((sum, student) => sum + Number(scores[student.id] || 0), 0) / enteredCount
    : 0;
      ...
      <div className="grid gap-5 lg:grid-cols-[1.5fr_1fr]">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="text-3xl font-black md:text-4xl">
                {assessmentLabel(assessmentType)} Results
              </h1>
              <p className="mt-2 max-w-2xl text-sm text-indigo-100">
                Enter the selected assessment component only. The official result sheet combines Test 1, Test 2, CA, Exam and the final total automatically.
              </p>
            </div>
        ...
        <section className="grid grid-cols-2 gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-900">
            <CalendarDays className="mb-2 h-5 w-5 text-indigo-600" />
          ...
          <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-900">
            ...
            <p className="text-xs text-slate-500">Current term</p>
            <p className="font-bold">{term?.term || session?.term_name}</p>
          </div>
          <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-900">
            <CheckCircle2 className="mb-2 h-5 w-5 text-emerald-600" />
            <p className="text-xs text-slate-500">Configured total</p>
            <p className="font-bold">{config?.total_max ?? 0} marks</p>
          </div>
          <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-900">
            <Users className="mb-2 h-5 w-5 text-amber-600" />
            <p className="text-xs text-slate-500">Register</p>
            <p className="font-bold">{students.length} students</p>
          </div>
        </section>
      </div>
      ...
      <section className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-[1fr_1fr_1fr_auto] dark:border-slate-700 dark:bg-slate-800">
        <label className="text-sm font-semibold">
          ...
          <select
            ...
            className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-3 py-3 dark:border-slate-600 dark:bg-slate-900"
          >
        ...
        <label className="text-sm font-semibold">
          ...
          <select
            ...
            disabled={!classId}
            className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-3 py-3 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-900"
          >
        ...
        <div className="rounded-2xl bg-indigo-50 p-3 dark:bg-indigo-950/30">
          <p className="text-xs font-semibold uppercase tracking-wide text-indigo-600">
            {assessmentLabel(assessmentType)} maximum
          </p>
          <p className="mt-1 text-2xl font-black text-indigo-700 dark:text-indigo-300">
            {maximum || '—'}
          </p>
          <p className="text-xs text-slate-500">Configured from Result Settings</p>
        </div>

        <button
          type="button"
          onClick={save}
          disabled={saving || loadingRegister || !students.length || !subjectId}
          className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-6 font-bold text-white shadow-lg shadow-indigo-200 transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_280px]">
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 p-5 dark:border-slate-700">
            <div>
              <h2 className="text-lg font-bold">Student broadsheet</h2>
              <p className="text-sm text-slate-500">
                {selectedClass?.name || 'Select a class'}
                {selectedSubject ? ` • ${selectedSubject.name}` : ''}
              </p>
            </div>
            <div className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold dark:bg-slate-900">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px] text-sm">
                <thead className="bg-slate-50 text-left dark:bg-slate-900">
                  <tr>
                    <th className="sticky left-0 bg-slate-50 p-4 dark:bg-slate-900">#</th>
                    <th className="p-4">Student</th>
                    <th className="p-4">Admission No.</th>
                    <th className="p-4">Score / {maximum || '—'}</th>
                    <th className="p-4">%</th>
                ...
                <tbody>
                  {students.map((student, index) => {
                    ...
                    const percentage = value == null || !maximum ? null : (value / maximum) * 100;

                    return (
                      <tr key={student.id} className="border-t border-slate-100 dark:border-slate-700">
                        <td className="sticky left-0 bg-white p-4 font-semibold dark:bg-slate-800">{index + 1}</td>
                        <td className="p-4 font-semibold">{student.last_name} {student.first_name}</td>
                        <td className="p-4 text-slate-500">{student.admission_number || '—'}</td>
                        <td className="p-4">
                          <input
                            ...
                            disabled={!subjectId}
                            className="w-32 rounded-xl border border-slate-300 bg-white px-3 py-2.5 font-semibold outline-none focus:border-indigo-500 dark:border-slate-600 dark:bg-slate-900"
                          />
                        </td>
                        ...
                        <td className="p-4 font-black">
                          {percentage == null ? '—' : grade(percentage)}
                        </td>
                        <td className="p-4">
                          {value == null ? (
                            <span className="text-slate-400">Pending</span>
                          ) : (
                            <span className="font-semibold text-emerald-600">Entered</span>
                          )}
                        </td>
                      </tr>
                    );
                  ...
                  {!students.length && (
                    <tr>
                      <td colSpan={7} className="p-14 text-center text-slate-500">
                        Select a class and subject to load the current session register.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <aside className="space-y-4">
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
            <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Entry overview</p>
            <div className="mt-4 space-y-4">
              <div><p className="text-xs text-slate-500">Assessment</p><p className="font-bold">{assessmentLabel(assessmentType)}</p></div>
              <div><p className="text-xs text-slate-500">Maximum mark</p><p className="text-2xl font-black">{maximum || '—'}</p></div>
              <div><p className="text-xs text-slate-500">Students entered</p><p className="text-2xl font-black">{enteredCount}</p></div>
              <div><p className="text-xs text-slate-500">Average entered score</p><p className="text-2xl font-black">{enteredCount ? averageEntered.toFixed(1) : '—'}</p></div>
            </div>
          </section>

          <section className="rounded-3xl border border-indigo-100 bg-indigo-50 p-5 dark:border-indigo-900/50 dark:bg-indigo-950/30">
            <p className="font-bold text-indigo-800 dark:text-indigo-200">Result calculation</p>
            <ul className="mt-3 space-y-2 text-sm text-indigo-900/80 dark:text-indigo-100/80">
              <li>• Test 1 + Test 2 = CA</li>
              <li>• CA + Exam = Total</li>
              <li>• Total is calculated against 100 marks</li>
              <li>• Grade and remark are generated automatically</li>
            </ul>
          </section>
        </aside>
      </div>

      ...
      {!rows.length ? <div className="p-10 text-center text-sm text-slate-500">No scored students were found for this selection.</div> : <div className="overflow-auto">
        <table className="min-w-[1550px] w-full text-xs">
          ...
          <tbody>{rows.map(row => <tr key={row.id} className="border-t dark:border-slate-700">
            ...
            <td className="sticky right-0 z-10 bg-white px-3 py-3 dark:bg-slate-800"><Link to="/admin/results/reportsheet" className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-2 text-xs font-medium text-indigo-700 hover:bg-indigo-100 dark:border-indigo-800 dark:bg-indigo-950/40 dark:text-indigo-300"><Eye className="h-3.5 w-3.5" /> View</Link></td>
          </tr>)}</tbody>
        </table>
      </div>}
    </section>
